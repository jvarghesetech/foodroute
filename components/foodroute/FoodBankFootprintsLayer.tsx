...
const SOURCE_ID = 'foodBank-footprints';
const LAYER_ID = 'foodBank-footprints-extrusion';

const CITY_IDS = ['toronto', 'waterloo', 'mississauga'];

interface FoodBankFootprintsLayerProps {
  map: mapboxgl.Map | null;
}

export default function FoodBankFootprintsLayer({ map }: FoodBankFootprintsLayerProps) {
    ...
    async function addLayer() {
      ...
      try {
        ...
        for (const cid of CITY_IDS) {
          try {
            const res = await fetch(`/map-data/foodBank-footprints-${cid}.geojson`);
            if (!res.ok) continue;
