...
export default function SimulationResultPanel({ result, foodBanks, proposedLabels = {} }: SimulationResultPanelProps) {
  const foodBankMap: Record<string, string> = {};
  for (const h of foodBanks) {
    foodBankMap[(h._id ?? h.id)?.toString()] = h.name;
  }

  const rows = Object.keys(result.before).map((id) => ({
    id,
    name: foodBankMap[id] ?? id,
        ...
        <table className="w-full text-[10px]">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              <th className="text-left px-3 py-2 font-bold text-slate-500 uppercase tracking-wide">FoodBank</th>
