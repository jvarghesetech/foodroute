...
import type { Blueprint, BlueprintMetadata, ProposedBuilding } from '@/lib/foodroute/blueprints';
...
export default function GovernmentSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener('foodroute:mapclick' as any, handleMapClick);
    return () => window.removeEventListener('foodroute:mapclick' as any, handleMapClick);
  }, []);
  ...
  const runSimulation = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/foodroute/simulate', {
        method: 'POST',
    ...
    <div className="civ-panel gov-panel">
      ...
      <div className="space-y-4">
        ...
        <div className="civ-glass rounded-2xl p-4 border border-sky-100/70 space-y-3">
          ...
          <p className="text-[11px] text-slate-500">
            {selectedBlueprint
              ? 'Click anywhere on the map to add a food bank. You can place multiple.'
              : 'Choose a blueprint above, then click on the map to place it.'}
          </p>
                        ...
                        {isExpanded && (
                          <div className="rounded-md bg-sky-50/80 border border-sky-100 p-2 space-y-1">
                            <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[10px]">
                              <MetaRow label="Capacity" value={meta.erBeds} />
