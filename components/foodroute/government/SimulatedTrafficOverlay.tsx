...
const TRAFFIC_SOURCE = 'foodroute-traffic-src';
...
export default function SimulatedTrafficOverlay({
  ...
  useEffect(() => {
    ...
    function addSimLayers() {
      ...
      // Group foodBank impact zones by post-simulation color
      ...
      for (const h of foodBanks) {
        ...
        const color = getColorForDemand(afterOcc);
      ...
      // Add zones around proposed foodBanks
      if (proposedAfter) {
    ...
    return cleanup;
  }, [map, simulationResult, foodBanks, proposedLocations]);
