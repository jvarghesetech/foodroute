...
import type { NeedsResponse, RouteResponse, ScoredFoodBank } from '@/lib/foodroute/types';
...
interface CivilianPanelProps {
  ...
  currentRecommendation?: RouteResponse & { activeRoute?: ScoredFoodBank } | null;
}
...
export default function CivilianPanel({ cityId, onRecommendation, currentRecommendation }: CivilianPanelProps) {
  ...
  const [needsResult, setNeedsResult] = useState<NeedsResponse | null>(null);
  ...
  useEffect(() => {
    if (currentRecommendation && currentRecommendation !== routeResult) {
      ...
      if (activeRoute) {
        const h = activeRoute.foodBank as { id?: string; _id?: string } | undefined;
        setActiveRouteId(h?.id ?? h?._id ?? null);
      } else {
        const rec = currentRecommendation.recommended as ScoredFoodBank | undefined;
        const h = rec?.foodBank as { id?: string; _id?: string } | undefined;
        setActiveRouteId(h?.id ?? h?._id ?? null);
      }
    }
  ...
  // When voice needs is complete, route the client
  const handleNeedsComplete = useCallback(
    async (needs: { urgency: 'critical' | 'high' | 'moderate'; reasoning: string; needs: { languages: string[]; dietaryNeeds: string[]; culturalPreference?: string; freeText?: string } | null }) => {
      setNeedsResult({ urgency: needs.urgency, reasoning: needs.reasoning });
      goTo('loading');
      setError(null);
      ...
      try {
        ...
        const routeRes = await fetch('/api/foodroute/route', {
          method: 'POST',
  ...
  const handleShowRoute = useCallback(
    (scored: ScoredFoodBank) => {
      if (!routeResult) return;
      const h = scored.foodBank as { id?: string; _id?: string };
    ...
    <div className="civ-panel">
        ...
        <AnimatePresence mode="wait" custom={direction}>
          ...
          {step === 'conversation' && (
            <motion.div key="conversation" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}>
              <VoiceIntake onNeedsComplete={handleNeedsComplete} />
            </motion.div>
          )}
          ...
          {step === 'loading' && (
            <motion.div key="loading" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}>
              <div className="flex flex-col items-center justify-center py-14">
                ...
                />
                <p className="text-sm font-bold text-slate-700 mt-4">Finding the best food bank for you...</p>
                <p className="text-xs text-slate-400 mt-1">Computing optimal route with live traffic</p>
              </div>
            </motion.div>
          )}

          {step === 'result' && needsResult && routeResult && (
      ...
      <div className="civ-progress">
        ...
        <p className="civ-progress-label">
          ...
          {step === 'conversation' && 'Step 2 of 3 — Tell us what you need'}
