...
import { ScoredFoodBank } from '@/lib/foodroute/types';
...
interface RoutingResultProps {
  ...
  onBack: () => void;
  onShowRoute?: (scored: ScoredFoodBank) => void;
  activeRouteId?: string | null;
}

const urgencyConfig = {
  critical: { gradient: 'from-red-600 to-red-700', label: 'CRITICAL', ring: 'ring-red-500/20' },
  high: { gradient: 'from-orange-500 to-orange-600', label: 'HIGH', ring: 'ring-orange-500/20' },
  moderate: { gradient: 'from-emerald-500 to-emerald-600', label: 'MODERATE', ring: 'ring-emerald-500/20' },
};

function FoodBankCard({ scored, rank, onShowRoute, isRouteActive }: { scored: ScoredFoodBank; rank: number; onShowRoute?: (scored: ScoredFoodBank) => void; isRouteActive?: boolean }) {
  const h = scored.foodBank;
  const isTop = rank === 1;

  return (
    <motion.div
      className={`civ-foodBank-card ${isTop ? 'civ-foodBank-card--top' : ''}`}
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        {(scored.languageMatch || scored.dietaryMatch) && <span className="civ-badge civ-badge--purple">Good Fit</span>}
      </div>
      ...
      <div className="flex items-center gap-2 text-[10px] text-slate-400 mt-2">
        ...
        <span>{scored.demandPct}% full</span>
      </div>
...
export default function RoutingResult({ urgency, reasoning, recommended, alternatives, onBack, onShowRoute, activeRouteId }: RoutingResultProps) {
  ...
  return (
    <div className="space-y-3.5">
      ...
      >
        <p className="text-[9px] font-bold text-white/70 uppercase tracking-[0.15em]">Needs Level</p>
      ...
      {/* Recommended */}
      <FoodBankCard scored={recommended} rank={1} onShowRoute={onShowRoute} isRouteActive={(recommended.foodBank?.id ?? (recommended.foodBank as any)?._id) === activeRouteId} />
            ...
            {showAlts && (
              ...
              >
                {alternatives.map((alt, i) => (
                  <FoodBankCard key={alt.foodBank.id || i} scored={alt} rank={i + 2} onShowRoute={onShowRoute} isRouteActive={(alt.foodBank?.id ?? (alt.foodBank as any)?._id) === activeRouteId} />
                ))}
              </motion.div>
            )}
          </AnimatePresence>
      ...
      {/* 211 Ontario — community & social services referral, including emergency food */}
      {urgency === 'critical' && (
        <motion.a
          href="tel:211"
        ...
        >
          ...
          Call 211 (Community &amp; Social Services)
        </motion.a>
      )}
