...
export default function FlowArcs({ map, foodBanks, proposedLocations, simulationResult }: FlowArcsProps) {
  useEffect(() => {
    if (!map || !simulationResult || foodBanks.length === 0 || proposedLocations.length === 0) return;

    const sourceId = 'flow-arcs';
    const layerId = 'flow-arc-lines';

    const features: GeoJSON.Feature<GeoJSON.LineString>[] = [];
    const foodBanksWithNegativeDelta = foodBanks.filter((h: any) => {
      const id = (h._id ?? h.id)?.toString();
      return simulationResult.delta && simulationResult.delta[id] < 0;
    });

    for (const h of foodBanksWithNegativeDelta) {
  ...
  }, [map, foodBanks, proposedLocations, simulationResult]);
