...
export function HospitalForm({ spec, onUpdate }: HospitalFormProps) {
  ...
  const fields: {
  ...
  }[] = [
    {
      ...
      description: 'Layout constraint for pallet placement and food safety control',
      warning: bedsExceedRooms ? 'Too many pallets for this storage bay count (max ~4 per bay)' : undefined,
    },
    {
      ...
      warning: bedBottleneck ? `Only ${usableInpatientBeds} of ${beds} pallets safely staffed` : undefined,
    },
    {
      label: 'Coordinators',
      key: 'hospitalDoctors',
    ...
    },
    {
      ...
      warning: nurseShortage ? `Need ${Math.ceil(nursesNeededForOR + nursesNeededForTrauma + nursesNeededForED)} just for cold storage + intake + distribution coverage` : undefined,
    },
    {
      ...
      description: `Each cold storage unit needs ~${NURSES_PER_OR} volunteers + ${RECOVERY_BAYS_PER_OR} staging bays`,
      warning: orUnderstaffed ? `Only ${usableORs} of ${ors} cold storage units have volunteer coverage` : undefined,
    },
    {
      ...
      warning: edUnderstaffed ? `Only ${usableEDBays} of ${edBays} distribution counters safely staffed` : undefined,
    },
    {
      ...
      description: 'Registration rooms for distribution — 1:1 volunteer ratio required',
      warning: traumaUnderstaffed ? `Only ${nursesForTrauma} of ${nursesNeededForTrauma} intake volunteers covered` : undefined,
    },
    {
      ...
      description: `Ground-floor bay parking (~${AMBULANCE_BAY_AREA} sq m each) — feeds distribution intake`,
      warning: ambulancesExceedED ? 'Delivery vehicles have no distribution counters to deliver food to' : undefined,
    },
  ];
      ...
      {hasActivity && (
        <div className="pt-4 mt-2 border-t-2 border-gray-200 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            {beds > 0 && rooms > 0 && (
              <div className="bg-gray-50 px-3 py-2 rounded-lg border border-gray-200">
                <span className="text-gray-500">Pallets / Bay</span>
