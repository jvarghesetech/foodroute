...
import FoodRouteMap from '@/components/foodroute/FoodRouteMap';
import ModeToggle from '@/components/foodroute/ModeToggle';
import DayNightToggle from '@/components/foodroute/DayNightToggle';
import CitySelector from '@/components/foodroute/CitySelector';
import GovernmentSidebar from '@/components/foodroute/government/GovernmentSidebar';
import CivilianPanel from '@/components/foodroute/civilian/CivilianPanel';
import TrafficTimeline from '@/components/foodroute/TrafficTimeline';
import { CITIES } from '@/lib/map-3d/cities';
import { createBlueprintFromBuilding, type Blueprint } from '@/lib/foodroute/blueprints';
import type { TimelinePrediction, RerouteAlert } from '@/lib/foodroute/trafficPrediction';
import type { ProposedBuilding } from '@/lib/foodroute/blueprints';
...
export default function MapPageContent() {
  ...
  const [recommendedFoodBank, setRecommendedFoodBank] = useState<any>(null);
  ...
  const handleMapClick = useCallback((lngLat: { lng: number; lat: number }, blueprint: Blueprint | null) => {
    if (mode === 'government' && blueprint) {
      ...
      window.dispatchEvent(new CustomEvent('foodroute:mapclick', { detail: { lngLat, blueprint } }));
    }
  }, [mode]);
  ...
  const handleRerouteRequest = useCallback(async (alert: RerouteAlert) => {
    ...
    setRecommendedFoodBank(swapped);
  }, [recommendedFoodBank]);

  const rec = recommendedFoodBank?.recommended;
  const activeRec = recommendedFoodBank?.activeRoute ?? rec;
  ...
  return (
    <div className={`fixed inset-0 overflow-hidden ${mapTheme === 'night' ? 'cp-night' : ''}`}>
      <FoodRouteMap
        ...
        recommendedFoodBank={recommendedFoodBank}
        ...
        <div className='cp-map-panel-wrap'>
          {mode === 'government' ? (
            ...
            <CivilianPanel
              ...
              currentRecommendation={recommendedFoodBank}
            />
          ) : (
            <div className="civ-panel civ-panel--pick-location">
              <div className="civ-header">
                ...
                <div>
                  <h2 className="civ-header-title">FoodRoute</h2>
                  <p className="civ-header-sub">Find the right food bank for your situation</p>
                </div>
              </div>
      ...
      {showTimeline && (
        <TrafficTimeline
          ...
          alternatives={recommendedFoodBank?.alternatives ?? []}
