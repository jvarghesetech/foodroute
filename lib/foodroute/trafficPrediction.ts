...
export interface RerouteAlertInput {
  ...
  recommended: {
    foodBank: { id?: string; _id?: string; name: string };
    demandPct: number;
    distanceKm: number;
...
export function generateRerouteAlerts(input: RerouteAlertInput): RerouteAlert[] {
  ...
  const bestAltName = bestAlt?.foodBank?.name ?? 'an alternative food bank';
  const foodBankId = recommended.foodBank?.id ?? (recommended.foodBank as any)?._id ?? 'unknown';
  ...
  if (prediction.avgCongestionLevel >= 2.5) {
    ...
    alerts.push({
      ...
      suggestedFoodBank: bestAltName,
    });
  }

  // 2. Demand surge — at over 90% of capacity
  if (recommended.demandPct > 90) {
    alerts.push({
      id: 'capacity',
      trigger: 'capacity',
      title: TRIGGER_ICONS.capacity,
      description: `${recommended.foodBank.name} is at ${Math.round(recommended.demandPct)}% of capacity. Expect extended wait times. ${bestAltName} may have shorter waits.`,
      severity: recommended.demandPct > 95 ? 'critical' : 'warning',
      suggestedFoodBank: bestAltName,
    });
  }

  // 3. Food bank closure — disabled (no real closure data source yet)
  const divSeed = `${foodBankId}-${hour}-diversion`;
  if (seededRandom(divSeed) < 0) {
    alerts.push({
      id: 'diversion',
      trigger: 'diversion',
      title: TRIGGER_ICONS.diversion,
      description: `${recommended.foodBank.name} has closed for the day and is not accepting new clients. Rerouting to ${bestAltName} is recommended.`,
      severity: 'critical',
      suggestedFoodBank: bestAltName,
    });
  }
  ...
  if (prediction.avgCongestionLevel >= 3 && recommended.distanceKm > 5) {
    const incidentSeed = `${foodBankId}-${hour}-incident`;
    if (seededRandom(incidentSeed) < 0.15) {
      alerts.push({
        id: 'incident',
        trigger: 'incident',
        title: TRIGGER_ICONS.incident,
        description: `A collision has been reported along your route to ${recommended.foodBank.name}. Consider rerouting to ${bestAltName} to avoid delays.`,
        severity: 'warning',
        suggestedFoodBank: bestAltName,
      });
    }
  ...
  if (bestAlt && recommended.totalEstimatedMinutes > 0) {
    ...
    if (pctFaster >= 0.15 && savings >= 5) {
      alerts.push({
        ...
        description: `${bestAlt.foodBank.name} is ~${Math.round(savings)} min faster overall (${bestAlt.totalEstimatedMinutes} min vs ${recommended.totalEstimatedMinutes} min).`,
        severity: 'warning',
        suggestedFoodBank: bestAlt.foodBank.name,
      });
    }
  }
